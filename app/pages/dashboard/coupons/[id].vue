...
watch(
  ...
  (value) => {
    ...
    form.slug = newSlug;
    form.canonical_url = newSlug ? `/coupon/${newSlug}` : "";
  },
);

const loadCoupon = async () => {
  const response = await couponStore.show(route.params.id);
  ...
  Object.assign(form, {
    ...
    expires_at: response.expires_at ? response.expires_at.split("T")[0] : "",
    ...
    is_exclusive: !!response.is_exclusive,
...
const submit = async () => {
  ...
  await couponStore.update(route.params.id, payload);
};
    ...
    >
      <div class="grid grid-cols-1 xl:grid-cols-3">
        ...
        <div class="p-5 md:p-6 bg-slate-50 space-y-6">
          ...
          <BaseButton :loading="couponStore.loading">Update</BaseButton>
        </div>
      </div>
    </form>
  </div>
